...
% This worksheet should be located within your MP data directory.
% The processing toolbox is (still) set up in a kind of stupid way as it
% expects the data to live under certain paths defined through experiment
% and cruise names. You will find out as you run into errors below if your
...
CalFile = 'NISKINe_M1_SN107_compass_cal.txt';
...
ac = MP_aqdp_correction('spintest_aqdp.txt');
...
gpng(sprintf('plots/file_size_overview_%s_%s_sn%s',...
...
MP_test_ctd_params(info,20, CTpar)
if 1
...
% MP_mag_pgrid(sn,cruise,mooring,361:368)
...
% MPall = MP_makeCTD(info,[45, 55],1,1);
